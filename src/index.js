...
const App = (() => {
  ...
  function captureButtons() {
    ...
    completionButtons.forEach(button => button.addEventListener('click', () => {
      toggleCompletion(button);
    }));
    completionButtons.forEach(button => button.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        button.click();
      }
    }));
    ...
    taskTitles.forEach(button => button.addEventListener('click', () => {
      readyTaskView(button);
    }));
    taskTitles.forEach(button => button.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        button.click();
      }
    }));
    ...
    taskDueDates.forEach(button => button.addEventListener('click', () => {
      readyTaskView(button);
    }));
  }
  ...
  function readyTaskView(button) {
    ...
    const task = tasks.find((task) => task.taskID === parseInt(taskID, 10));
